...
/** Close the currently open graphics window.  */
...
/** Determine whether the given key has been pressed
 @param key A string representing the key to check. If it is one character long,
 it is interpreted as the name of the key itself. For arrow keys,
 use "up", "down", "left" or "right"
 @return true if the given key is down, false if it is not */
bool KeyWasPressed(string key);

/** Waits for the user to type a single key in the graphics window.
