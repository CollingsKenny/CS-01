...
{
    if (!gpixels)
        Error("Graphics windows must be open before any drawing, mouse or GetChar() commands.");
}
...
void GraphicsWindow(int width, int height, string color)
{
    ...
    if (gpixels != NULL)
    {
        cout << "*** GraphicsWindow() called when window is already open - discarding old window.\n";
        delete gdisplay;
        delete gpixels;
    }
...
void CloseGraphicsWindow()
{
    ...
    {
        cout << "** There is no graphics window open, ignoring CloseGraphicsWindow().\n";
    }
}
...
bool KeyWasPressed(string key)
{
    ...
    if (gdisplay->key() == what)
    {
        last = what;
        return true;
    }
    else if (gdisplay->released_key() == what)
    {
        gdisplay->unrelease_key();
        if (last == what)
        {
            last = 0;
            return false;
